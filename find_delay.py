"""Contains 5 functions, all directed at calculating the delay between two arrays:
* filter_frequencies applies a low, high or band-pass filter on an array.
* get_number_of_windows gets how many windows are necessary given an array length, a window size and an overlap.
* get_envelope returns the envelope of a time series.
* resample resamples a time series.
...
Current version: 1.0 (2024-04-12)
"""
...
def filter_frequencies(array, frequency, filter_below=None, filter_over=None, verbosity=1):
...
def get_envelope(array, frequency, number_of_windows=100, overlap_ratio=0.5, filter_below=None, filter_over=None,
                 verbosity=1):
    ...
    if filter_below is not None or filter_over is not None:
        envelope = filter_frequencies(envelope, frequency, filter_below, filter_over, verbosity)
...
def resample(array, original_frequency, resampling_frequency, number_of_windows=100, overlap_ratio=0.5,
...
def find_delay(array_1, array_2, freq_array_1=1, freq_array_2=1, compute_envelope=True, number_of_windows_env=10000,
    ...
    if compute_envelope:
        ...
        envelope_1 = get_envelope(array_1, freq_array_1, number_of_windows_env, overlap_ratio_env, filter_below,
                                  filter_over, verbosity)
        if verbosity > 0:
            print("Getting the envelope from array 2...")
        envelope_2 = get_envelope(array_2, freq_array_2, number_of_windows_env, overlap_ratio_env, filter_below,
                                  filter_over, verbosity)
        if verbosity > 0:
    ...
    if resampling_rate is not None:
        ...
        y1 = resample(envelope_1, freq_array_1, resampling_rate, number_of_windows_res, overlap_ratio_res,
                      resampling_mode, verbosity)
        if verbosity > 0:
            print("Resampling array 2...")
        y2 = resample(envelope_2, freq_array_2, resampling_rate, number_of_windows_res, overlap_ratio_res,
                      resampling_mode, verbosity)
        if verbosity > 0:
            print("Resampling done.\n")
    else:
    ...
    if plot_figure or path_figure is not None:
        ...
        #     fig, ax = plt.subplots(int(np.ceil(number_of_plots / 2)), 2, constrained_layout=True)
        #     print("1", fig, ax)
        # else:
        ...
        if plot_intermediate_steps:
            ax[i // 2][i % 2].set_title("Array 1: " + str(round(freq_array_1, 2)) + " Hz")
            ax[i // 2][i % 2].plot(np.arange(0, len(array_1)) / freq_array_1, array_1)
            i += 1

            ax[i // 2][i % 2].set_title("Array 2: " + str(round(freq_array_2, 2)) + " Hz")
            ...
            if compute_envelope:
                ...
                title = "Envelope Array 1: " + str(number_of_windows_env) + " w, " + str(overlap_ratio_env) + " o"
                if filter_below is not None or filter_over is not None:
                ...
                title = "Envelope Array 2: " + str(number_of_windows_env) + " w, " + str(overlap_ratio_env) + " o"
                if filter_below is not None or filter_over is not None:
                    title += " · Band-pass [" + str(band_pass_low) + ", " + str(band_pass_high) + "]"
                ax[i // 2][i % 2].set_title(title)
            ...
            if resampling_rate is not None:
                ...
                title = "Resampled " + env_or_array + " 1: " + str(resampling_rate) + " Hz, " + \
                        str(number_of_windows_res) + " w, " + str(overlap_ratio_res) + " o"
                ax[i // 2][i % 2].set_title(title)
                ax[i // 2][i % 2].plot(np.arange(0, len(y1)) / resampling_rate, y1)
                i += 1

                title = "Resampled " + env_or_array + " 2: " + str(resampling_rate) + " Hz, " + \
                        str(number_of_windows_res) + " w, " + str(overlap_ratio_res) + " o"
                ax[i // 2][i % 2].set_title(title)
        ...
        # ax[i // 2][i % 2].plot(np.arange(0, len(y1))/freq_array_1, y1, color="#04589388", linewidth=1)
        ax[i // 2][i % 2].plot(np.arange(0, len(array_1)) / freq_array_1, array_1, color="#04589388", linewidth=1)

        # resampled_timestamps_array2 = np.arange(0, len(y2)) / freq_array_1 + delay / freq_array_1
        # resampled_timestamps_array2 = resampled_timestamps_array2[:len(y2)]
        # ax[i // 2][i % 2].plot(resampled_timestamps_array2, y2, color="orange", linewidth=2)
